...
fn main() {
    ...
    // instruments.push(Instrument::from_oscillators(oscillators_1));
    // instruments.push(Instrument::from_oscillators(oscillators_2));
    // let mut multi_instrument = MultiInstrument::from_instruments(instruments);
    // let note = Note::from(frequency, volume, duration_ms);
    // multi_instrument.add_note_to_channels( note);
    // multi_instrument.play();
